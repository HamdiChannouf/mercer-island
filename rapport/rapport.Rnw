...
% {Données Géographiques}
<<prix, echo=FALSE, fig.cap='Prix', fig.subcap=c('Répartition en échelle log', 'Courbe de Lorentz'), out.width='.5\\linewidth'>>=
#ggplot(D, aes(price)) + geom_histogram(binwidth=10^5)
ggplot(D, aes(log(price))) + geom_histogram(bins=30)

sortedPrice <- sort(D$price)
qplot(1:nrow(D)/nrow(D), cumsum(sortedPrice) / sum(sortedPrice), xlab="Pourcentage de la population", ylab="Richesse cumulée")
@ 
