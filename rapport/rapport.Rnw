...
D$yr_renovated <- as.Date(paste(D$yr_renovated, "01-01", sep = "-"))

D$date <- as.integer(D$date)
D$yr_built<- as.integer(D$yr_built)
D$yr_renovated<- as.integer(D$yr_renovated)

D$d_built <- D$date - D$yr_built
D$d_renovated <- D$date - D$yr_renovated
...
<<>>=
map_plot <- ggplot(map_mercer, aes(x=long, y=lat)) + geom_polygon(fill="lightblue") 

map_plot + geom_point(data=as.data.frame(pts@coords), aes(long, lat))
@ 
...
affiche_lorentz <- function(v)
{
    ...
    ggplot(data.frame(lp=seq(length(v))/length(v),
        ...
        geom_label(data=data.frame(x=0.5, y=0.425, label=paste("italic(G) ==", round(sqft_living_G, 2))), aes(x=x, y=y, label=label), parse=TRUE, size=9)
}
@ 
...
<<sqft_living, fig.cap='Surface habitée', fig.subcap=c('Répartition en échellle linéaire', 'Courbe de Lorentz'), out.width='.5\\linewidth'>>=

ggplot(M, aes(sqft_living)) + geom_histogram(bins=30)
...
<<correlation>>=
corrplot(cor(select(M, -c(view, condition, grade, lat, long, d_built, d_renovated, zipcode, waterfront))), type="upper", order="hclust", addCoef.col="black")
@ 
