...
```
Une valeur anormalement grande a été retiré en raison d’une valeur de $sqft\_lot$ très éloignée du nuage (supérieur à 75000).
...
```
Une valeur anormalement grande a été retiré du second graphe en raison d’une valeur de $sqft\_lot$ très éloignée du nuage (supérieur à 75000).
...
```{r}
pca_ind <- data.frame(x=pca$ind$coord[,1], y=pca$ind$coord[,2])
ggplot(pca_ind) +
...
plot_grid(
plot_grid(
    ggplot(regression) + geom_point(aes(regression$fitted.values,
