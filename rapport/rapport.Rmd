...
map_sqft_basement <- map_plot + geom_point(data=as.data.frame(pts), aes(long, lat, fill=sqrt(sqft_basement)), shape=23, size=3) + scale_shape_identity()
```
...
```{r}
map_date <- map_plot + geom_point(data=as.data.frame(pts), aes(long, lat, fill=date), shape=23, size=3) + scale_shape_identity()
```
...
```{r}
plot_grid(
    ggplot(M, aes(sqrt(sqft_living15))) + geom_histogram(bins=30),
...
```{r}
plot_grid(
    ggplot(M, aes(sqrt(sqft_lot))) + geom_histogram(bins=30),
...
```{r}
plot_grid(
    ggplot(M, aes(sqrt(sqft_lot15))) + geom_histogram(bins=30),
