...
### Variables continues
```{r}
sommaire <- sapply(select(M, -c(id, grade, condition, zipcode, waterfront, floors, date, yr_built, yr_renovated, bedrooms, view)), summary)
...
```{r}
plot_continuous <-  M %>% select(-c(id, grade, condition, floors, zipcode, waterfront, date, yr_built, yr_renovated, bedrooms, view)) %>% mutate(log10_price = log10(price)) %>% select(-price)
...
```{r}
plot_discrete <- M %>% select(c(grade, condition, floors, waterfront, bedrooms, view)) %>% mutate_at(.funs = as.factor, .vars = vars(floors, bedrooms, view))

ggplot(data=melt(plot_discrete, id.var=NULL), mapping = aes(x = value)) + geom_bar() + facet_wrap(~variable, scale = 'free_x')
...
plot_grid(
    ...
    ggtitle("Initialement"),
    affiche_lorentz(M$sqft_living15) +
    draw_plot(ggplot(M) +
              geom_histogram(aes(sqft_living15)) +
              theme(axis.text=element_text(size=5)), 0.5, 0, 0.5, 0.5) +
    ggtitle("En 2015")
)
```
...
plot_grid(
    ...
    M %>% filter(sqft_lot < 75000) %>% ggplot() + geom_point(aes(sqft_lot, log10(price)))
)
```
Un point a été retiré du second graphe en raison d’une valeur de $sqft\_lot$ très éloignée du nuage (supérieur à 75000).


### Surface du sous-sol et supérieure
```{r}
plot_grid(
    M %>% ggplot() + geom_point(aes(sqft_basement, log10(price))),
    M %>% ggplot() + geom_point(aes(sqft_above, log10(price)))
)
```
...
#### Date de vente
```{r}
M %>% ggplot()+ geom_boxplot(aes(format(as.Date(cut(M$date, "month")), "%B"),
                                 log10(price))) +
...
Un point a été retiré du premier graphe en raison d'une valeur de $sqft\_lot$ très éloignée du nuage (supérieur à 75000).
...
```{r}
plot_grid(
    M %>% ggplot() + geom_boxplot(aes(condition, sqft_lot15)),
    M %>% ggplot() + geom_boxplot(aes(condition, sqft_lot))
)
```
...
## Logements au prix anormalement bas

On procède à une régression linéaire sur le log décimale du prix suivant une sélection de variables 
considérées significatives (statistiquement). On remarque des valeurs de prix très surrestimées.

```{r}
regression <- lm(log10(price)~
                     sqft_living +
#                     sqft_living15 +
                     sqft_basement +
                     bathrooms +
...
regression$coefficients %>% as.matrix %>% t %>% kable %>%
    kable_styling("striped", full_width = F)
```

```{r}
res_map <- map_plot + geom_point(data=as.data.frame(pts), aes(long, lat,
                                             fill=cut(regression$residuals,
                                                      breaks=c(-Inf,
                                                               -0.2,
                                                               -0.1,
                                                               0.1,
    ...
    scale_fill_brewer(name = "Résidus", palette = "Set1")

res_point <-  ggplot(regression) + geom_point(aes(regression$fitted.values,
                                    regression$residuals,
                                    color=cut(regression$residuals,
                                              breaks=c(-Inf,
                                                       -0.2,
                                                       -0.1,
                                                       0.1,
                                                       0.2,
                                                       Inf)))) +
    scale_color_brewer(name = "Résidus", palette = "Set1")

plot_grid(
plot_grid(
    res_point + 
    theme(legend.position = "none"),
    res_map + theme(legend.position = "none"),
    align = "vh",
    hjust = -1,
    nrow = 1
),
...
```{r}
plot_grid(
    ggplot(M) + geom_point(aes(sqft_living, sqft_basement), alpha=0.5),
    map_plot +
