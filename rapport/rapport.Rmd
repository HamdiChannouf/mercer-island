...
```{r}
plot_grid(
    affiche_lorentz(M$sqft_living) + draw_plot(ggplot(M) + geom_histogram(aes(sqft_living))+ theme(axis.text=element_text(size=10)), 0.5, 0, 0.5, 0.5),
    affiche_lorentz(M$sqft_living15) + draw_plot(ggplot(M) + geom_histogram(aes(sqft_living15))+ theme(axis.text=element_text(size=10)), 0.5, 0, 0.5, 0.5)
)
```
...
```{r}
plot_grid(
    affiche_lorentz(M$sqft_lot) + draw_plot(ggplot(M) +
                                            geom_histogram(aes(sqft_lot))+ theme(axis.text=element_text(size=10)), 0.5, 0, 0.5, 0.5),
    affiche_lorentz(M$sqft_lot15) + draw_plot(ggplot(M) +
                                              geom_histogram(aes(sqft_lot15))+ theme(axis.text=element_text(size=10)), 0.5, 0, 0.5, 0.5)
)
```
...
plot_grid(
    ...
    M %>% ggplot() + geom_histogram(aes((sqft_living15 - sqft_living)/sqft_living)) + ggtitle("Relatif")
)
...
```{r}
pca <- PCA(plot_continuous)
```

### Projection des variables sur le plan d'inertie maximum
```{r}
pca_var<- data.frame(x=pca$var$coord[,1], y=pca$var$coord[,2])
ggplot(pca_var)  +
    ...
    geom_text(aes(x, y, label=rownames(toot)), vjust=0) +
    coord_fixed()
```
