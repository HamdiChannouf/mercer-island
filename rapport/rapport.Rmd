...
map_plot + geom_point(data=as.data.frame(pts), aes(long, lat, fill=sqft_living15/sqft_basement), shape=23, size=3) + scale_shape_identity()
```
...
```{r}
ggplot(M) + geom_point(aes(sqft_living15, log10(price)))
```

```{r}
ggplot(M) + geom_boxplot(aes(factor(bedrooms), bathrooms))
ggplot(M) + geom_point(aes(sqft_living, bathrooms))

ggplot(M) + geom_boxplot(aes(factor(bedrooms), price))

ggplot(M) + geom_point(aes(sqft_living15, sqft_basement))
```
